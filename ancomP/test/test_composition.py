...
from ancomP.linalg.composition import (perturb, perturb_inv, power, clr, clr_inv,
                                       ilr, ilr_inv,
                                       centralize, variation_matrix, total_variation,
                                       multiplicative_replacement, closure)
...
class CompositionTests(unittest.TestCase):
    ...
    def testclosure(self):
        ...
        npt.assert_allclose(closure(self.data2),
                            np.array([.2, .2, .6]))
        npt.assert_allclose(closure(self.data6),
                            np.array([[.2, .2, .6],
                                      [.4, .4, .2]]))
    ...
    def test_perturb(self):
        ...
        pmat = perturb(closure(self.data6), np.array([1, 1, 1]))
        npt.assert_allclose(pmat,
                            np.array([[.2, .2, .6],
                                      [.4, .4, .2]]))
    ...
    def test_power(self):
        ...
        pmat = power(closure(self.data6), 2)
        npt.assert_allclose(pmat,
                            np.array([[.04/.44, .04/.44, .36/.44],
                                      [.16/.36, .16/.36, .04/.36]]))
    ...
    def test_perturb_inv(self):
        ...
        npt.assert_allclose(pmat,
                            np.array([[.2, .2, .6],
                                      [.4, .4, .2]]))
        pmat = perturb_inv(closure(self.data6), np.array([.1, .1, .1]))
        imat = perturb(closure(self.data1), np.array([10, 10, 10]))
        npt.assert_allclose(pmat, imat)
        ...
        npt.assert_allclose(cmat,
                            np.array([[0.22474487, 0.22474487,
                                      ...
                                      [0.41523958, 0.41523958,
                                       0.16952085]]))
        cmat = centralize(closure(self.data6))
        npt.assert_allclose(cmat,
                            np.array([[0.22474487, 0.22474487,
                                       0.55051026],
                                      [0.41523958, 0.41523958,
